...
public class DeviceController extends BaseController {
    ...
    public ResponseEntity changeUser(HttpServletRequest servletRequest,
        ...
        try {
            DeviceChangeUserRequest request = BaseEntity.objectV1_0(ApplicationUtility.decrypt(appKey, requestBody),
                    DeviceChangeUserRequest.class);
            ...
            User oldUser = this.getCore_().getDao().find(User.class, "phone",
                    request.getOldPhoneNum().length() == 10 ? "7" + request.getOldPhoneNum() : request.getOldPhoneNum());
            if (oldUser == null)
                throw new UserNotFoundException(HttpStatus.OK.value());

            User newUser = this.getCore_().getDao().find(User.class, "phone",
                    request.getNewPhoneNum().length() == 10 ? "7" + request.getNewPhoneNum() : request.getNewPhoneNum());
            if (newUser == null)
                throw new UserNotFoundException(HttpStatus.OK.value());
